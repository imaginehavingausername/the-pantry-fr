...
interface NewItem {
  ...
  confidence: "high" | "medium" | "low";
  include: boolean; // review-page-only field
  expirationDate: string; // optional; an empty string is stored as no expiration date
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const attemptsMsg = Array.isArray(attempts)
          ...
          : "";
        throw new Error((data.error || "Scan failed.") + attemptsMsg);
      }
      ...
      const payload = {
        ...
        new_items: newItems
          ...
          .map((n) => ({
            name: n.name,
            quantity: n.quantity,
            expirationDate: n.expirationDate ? new Date(n.expirationDate).toISOString() : null,
          ...
          {scanned && (
            <div className="flex flex-col gap-4">
              ...
              {newItemsMissingRequiredFields && (
                <div className="text-destructive">Every included new item needs a placement before you can confirm.</div>
              )}
